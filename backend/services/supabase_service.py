...
class SupabaseService:
    ...
    def get_available_chapters(self) -> List[str]:
        """Get all available chapter names from chemistry_chapters table"""
        try:
            response = self.supabase.table('chemistry_chapters').select('chapter_name').execute()
            if response.data:
                # Extract chapter_name from each record and return as list
                chapters = [chapter.get('chapter_name') for chapter in response.data if chapter.get('chapter_name')]
                return chapters
            return []
        except Exception as e:
            print(f"Error fetching available chapters: {str(e)}")
            return []
